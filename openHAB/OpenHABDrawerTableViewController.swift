    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let sitemapsUrl = Endpoint.sitemaps(openHABRootUrl: openHABRootUrl).url {
            ...
            operation.setCompletionBlockWithSuccess({ operation, responseObject in
                ...
                let sitemaps = deriveSitemaps(response, version: self.appData?.openHABVersion)

                // Don't show the `_default` sitemap in the drawer if it's the only one.
                if sitemaps.count == 1 && sitemaps.first!.name == "_default" {
                  self.sitemaps = []
                } else {
