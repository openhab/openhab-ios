...
struct SwitchCellProvider: WidgetCellProvider {
    static var reuseIdentifier: String { "SwitchUITableViewCell" }

    static func dequeue(from tableView: UITableView, at indexPath: IndexPath) -> UITableViewCell {
        tableView.dequeueReusableCell(for: indexPath) as SwitchUITableViewCell
    }

    static func configure(cell: UITableViewCell, for widget: OpenHABWidget, controller: OpenHABSitemapViewController) {
        guard let cell = cell as? SwitchUITableViewCell else { return }
        cell.widget = widget
        cell.displayWidget()
