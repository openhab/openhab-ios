        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                Button {
                    ...
                    Label(String(localized: "command_item_docs_link"), systemSymbol: .arrowUpRightSquare)
                }
            }
            .padding()
...
struct HomeSettingsView: View, SettingsSheetView {
    struct SettingsSnapshot: Equatable {
        ...
        var defaultMainUIPath = ""
        var alwaysAllowWebRTC = true
        var sitemapForWatch = ""
        var sitemapForCarPlay = ""
        var localConnectionConfig: ConnectionConfiguration = .init(url: "", username: "", password: "")
        var remoteConnectionConfig: ConnectionConfiguration = .init(url: "", username: "", password: "")
        var sseCommandItem = ""
        var homeName = ""
        var disableRemoteConnection = false
    ...
    var body: some View {
        ...
        .task {
            ...
            let homePrefs: HomePreferences = if let homeId, let stored = await Preferences.shared.storedHomeWithCredentials(forId: homeId) {
                stored
            } else {
    ...
    private func inlineSectionRow(_ section: MenuSection, isVisible: Bool) -> some View {
        HStack(spacing: 12) {
            Button {
                ...
                Image(systemSymbol: isVisible ? .eyeFill : .eyeSlash)
    ...
    private func saveSettings() async {
        ...
        let targetId: UUID = if let id = capturedHomeId {
            id
        } else {
            await (Preferences.shared.currentHomePreferences).id
        }
...
extension HomeSettingsView {
    ...
    private var avatarPickerButton: some View {
        ...
        let icon = current.avatarMode?.icon ?? HomeAvatarView.defaultIcon
        ...
        return Button {
            ...
            HomeAvatarView(photo: displayImage, icon: icon, color: avatarColor, size: 72)
                .overlay(alignment: .bottomTrailing) {
                    Image(systemSymbol: showAvatarPicker ? .checkmarkCircleFill : .pencilCircleFill)
                        .font(.title3)
    ...
    private var iconPickerRow: some View {
        let hasPhoto = if case .image = current.avatarMode { true } else { false }
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 10) {
                    ForEach(HomeAvatarView.availableIcons, id: \.self) { icon in
                        let isSelected = !hasPhoto && (current.avatarMode?.icon ?? HomeAvatarView.defaultIcon) == icon
                        Button {
                            withAnimation(.easeInOut(duration: 0.2)) {
                                let color = current.avatarMode?.colorHex ?? HomeAvatarView.colorPalette[0]
                                current.avatarMode = .icon(name: icon.rawValue, color: color)
    ...
    private var settingsColorPicker: some View {
        ...
        let base = ColorPicker("", selection: Binding(
            ...
            set: { current.avatarMode = (current.avatarMode ?? .icon(name: HomeAvatarView.defaultIcon.rawValue, color: "")).withColor($0.hexString) }
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 8) {
                    ForEach(HomeAvatarView.colorPalette, id: \.self) { hex in
                        ...
                        Button {
                            current.avatarMode = (current.avatarMode ?? .icon(name: HomeAvatarView.defaultIcon.rawValue, color: "")).withColor(hex)
                        } label: {
                            Circle()
                                .fill(color)
...
extension MenuSection {
    var displayName: String {
        switch self {
        case .mainUI: String(localized: "Main UI")
        case .sitemaps: String(localized: "Sitemaps")
        case .tiles: String(localized: "Tiles")
        case .system: String(localized: "System & App")
        }
    }
}
