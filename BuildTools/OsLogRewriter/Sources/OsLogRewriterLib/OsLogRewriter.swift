...
final class OsLogRewriter: SyntaxRewriter {
    ...
    func simplifyStringInterpolation(_ expr: ExprSyntax) -> ExprSyntax {
        ...
        if stringLiteral.segments.count == 1 {
            ...
            if case .expressionSegment(let expressionSegment) = stringLiteral.segments.first,
            ...
            if case .stringSegment(let firstSegment) = segments[0],
               case .expressionSegment(let middleSegment) = segments[1],
               case .stringSegment(let lastSegment) = segments[2],
