...
public enum SitemapSseError: Error {
    case unsupported
}
...
public actor SitemapEventStream {
    ...
    public func setTarget(sitemap: String, pageId: String) async {
        isStopped = false
    ...
    public func startMonitoringNetworkIfNeeded() {
        ...
        Task {
            for await conn in await NetworkTracker.shared.activeConnectionStream() {
                await self.updateConnection(conn)
            }
        }
    }
...
public extension SitemapEventStream {
    static let shared = SitemapEventStream()

    nonisolated static func startMonitoringNetwork() {
        Task {
            await shared.startMonitoringNetworkIfNeeded()
        }
    }
}
